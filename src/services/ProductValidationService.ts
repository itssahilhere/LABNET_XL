...
export async function validateProductRow(
    ...
    if (row.Fancy_Color_Intensity && !isValidFancyColorIntensity(row.Fancy_Color_Intensity.toString())) {
    ...
    if (row.Depth_Percentage && (Number(row.Depth_Percentage) < 0 || Number(row.Depth_Percentage) > 100)) {
        errors.depth_percentage = 'Depth percentage must be between 0 and 100';
    }
    
    if (row.Table_Percentage && (Number(row.Table_Percentage) < 0 || Number(row.Table_Percentage) > 100)) {
