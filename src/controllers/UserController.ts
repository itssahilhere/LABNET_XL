...
export class UserController {
  ...
  public async register(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!req.body.fileDetails || !Array.isArray(req.body.fileDetails) || req.body.fileDetails.length === 0) {
        errors.push('ID proof file is required');
      }
      ...
      if (req.body.fileDetails && req.body.fileDetails.length > 0) {
        try {
          ...
          return sendErrorResponse(
            res,
            ErrorResponses.BAD_REQUEST(uploadError.message || 'File upload failed')
          );
        }
      }
  ...
  public async login(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!email || !password) {
        return sendErrorResponse(
          res,
          ErrorResponses.VALIDATION_ERROR('Email and password are required')
        );
      }
      ...
      return sendSuccessResponse(
        res,
        SuccessResponses.OK('Login successful', {
          user: {
            _id: user._id,
            name: user.name,
            email: user.email,
  ...
  public async getProfile(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return sendSuccessResponse(
        res,
        SuccessResponses.OK('Profile retrieved successfully', {
          _id: user._id,
          name: user.name,
          email: user.email,
          company_name: user.company_name,
  ...
  public async updateProfile(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const updateKeys = Object.keys(updates).filter(
        key => key !== 'fileDetails' && key !== 'id_proof'
      );
      ...
      if (updates.id_proof) {
        return sendErrorResponse(
          res,
          ErrorResponses.BAD_REQUEST('ID proof cannot be updated directly. Please use file upload.')
        );
      }
      ...
      return sendSuccessResponse(
        res,
        SuccessResponses.OK('Profile updated successfully', {
          _id: updatedUser._id,
  ...
  public async getDashboard(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return sendSuccessResponse(
        res,
        SuccessResponses.OK('Dashboard statistics retrieved successfully', dashboardStats)
      );
    } catch (error: any) {
      logError({
        userId: req.user?._id || 'unknown',
