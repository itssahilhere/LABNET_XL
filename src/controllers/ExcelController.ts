...
export class ExcelController {
    ...
    private checkPackage(req: Request, res: Response): boolean {
        ...
        if (!user?.package?.end_date || new Date(user.package.end_date) < new Date()) {
            sendErrorResponse(res, ErrorResponses.FORBIDDEN('Your package has expired. Please buy a package first.'));
            return false;
        }
        return true;
    }
    ...
    public uploadExcel = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            if (!fileData || !fileData.base64 || !fileData.fileName) {
                return sendErrorResponse(res, ErrorResponses.BAD_REQUEST('Excel file is required with base64 content and fileName'));
            }

            // Validate file type
            const allowedExtensions = ['.xls', '.xlsx'];
            const fileExtension = fileData.fileName.substring(fileData.fileName.lastIndexOf('.')).toLowerCase();
            
            if (!allowedExtensions.includes(fileExtension)) {
                return sendErrorResponse(res, ErrorResponses.BAD_REQUEST('Only Excel files (.xls, .xlsx) are allowed'));
            }
            ...
            if (contentType && !allowedMimes.includes(contentType)) {
                return sendErrorResponse(res, ErrorResponses.BAD_REQUEST(`Invalid content type: ${contentType}. Only Excel files are allowed.`));
            }
