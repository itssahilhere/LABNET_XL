...
export class ApiController {
  ...
  private async findUserProduct(userId: string, productId: string) {
    return await Product.findOne({
      $and: [{ seller_id: userId }, { $or: [{ _id: productId }, { pid: productId }] }],
    });
  }
    ...
    try {
      ...
      if (packageCheck.valid) {
        return res.status(400).json({
          status: false,
          message:
            'You already have an active package. Please wait for it to expire before purchasing a new one.',
        });
      }
        ...
        if (packEndDate > today) {
          currentPackage = {
            ...
            days_remaining: Math.ceil(
              (packEndDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
            ),
          };
        }
      }
  ...
  public async getPublicProducts(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const today = new Date();
      const usersWithActivePackages = await (
