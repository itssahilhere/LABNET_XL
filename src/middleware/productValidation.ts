...
export const validateProductCreation = (req: Request, res: Response, next: NextFunction) => {
  ...
  for (const field of requiredFields) {
    if (
      !data[field as keyof IProductCreateRequest] &&
      data[field as keyof IProductCreateRequest] !== 0
    ) {
  ...
  if (
    data.symmetry &&
    !(ALLOWED_CUT_POLISH_SYMMETRY as readonly string[]).includes(data.symmetry)
  ) {
  ...
  if (
    data.fluorescence &&
    !(ALLOWED_FLUORESCENCE as readonly string[]).includes(data.fluorescence)
  ) {
  ...
  if (
    ...
    !(ALLOWED_FANCY_COLOR_INTENSITY as readonly string[]).includes(data.fancy_color_intensity)
  ) {
  ...
  if (
    ...
    !(ALLOWED_FANCY_COLORS as readonly string[]).includes(data.fancy_color_overtone)
  ) {
...
export const validateProductUpdate = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (
    data.depth_percentage !== undefined &&
    (typeof data.depth_percentage !== 'number' ||
      data.depth_percentage < 0 ||
      data.depth_percentage > 100)
  ) {
  ...
  if (
    data.table_percentage !== undefined &&
    (typeof data.table_percentage !== 'number' ||
      data.table_percentage < 0 ||
      data.table_percentage > 100)
  ) {
  ...
  if (
    ...
    (typeof data.price_per_carat !== 'number' || data.price_per_carat < 0)
  ) {
