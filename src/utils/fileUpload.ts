...
export async function uploadFiles(
  ...
  for (const file of fileObjects) {
    if (!file.base64 || !file.fileName) {
      throw {
        code: 400,
        message: 'Missing file data. Each file must have base64 content and fileName.',
      };
    }
    ...
    try {
      ...
      if (!file.base64.startsWith('data:')) {
        throw {
          code: 400,
          message: `Invalid base64 format for "${file.fileName}". Expected format: data:type;base64,content`,
        };
      }
      ...
      if (!file.base64.includes(';base64,')) {
        throw {
          code: 400,
          message: `Invalid base64 format for "${file.fileName}". Missing base64 encoding declaration.`,
        };
      }
      ...
      if (base64Parts.length !== 2 || !base64Parts[1]) {
        throw {
          code: 400,
          message: `Invalid base64 format for "${file.fileName}". Malformed base64 content.`,
        };
      }
      ...
      if (!/^[A-Za-z0-9+/]*={0,2}$/.test(base64Content)) {
        throw {
          code: 400,
          message: `Invalid base64 content for "${file.fileName}". Contains invalid characters.`,
        };
      }
      ...
      if (!contentType || contentType.trim() === '') {
        throw {
          code: 400,
          message: `Invalid file format for "${file.fileName}". Unable to determine content type from: ${base64Header}`,
        };
      }
      ...
      if (!contentType.includes('/')) {
        throw {
          code: 400,
          message: `Invalid content type for "${file.fileName}": ${contentType}. Expected format: type/subtype`,
        };
      }
      ...
      throw {
        code: 500,
        message: `Failed to upload file "${file.fileName}": ${error.message || 'Unknown error'}`,
      };
    }
  }
